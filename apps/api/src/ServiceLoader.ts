...
export class ServiceLoader {
  ...
  protected instantiate(): void {
    ...
    this.services = this.instantiateFromConfig<ServiceConfig, ProductService>(
      this.serviceConfigs,
      () => new ProductService(),
    );
    ...
    this.families = this.instantiateFromConfig<FamilyConfig, ProductFamily>(
      this.familyConfigs,
      (config) => new ProductFamily(config),
    );

    // Instantiate all attributes
    console.debug('Instantiating attribute instances...');
    this.attributes = Object.fromEntries(
      allAttributes.map((attribute) => [attribute.getName(), attribute]),
    );
    ...
    this.filters = this.instantiateFromConfig<FilterConfig, ProductAttrFilter>(
      this.filterConfigs,
      (config) => new ProductAttrFilter(config),
    );

    // Instantiate all assets
    console.debug('Instantiating asset instances...');
    this.assets = this.instantiateFromConfig<AssetConfig, ProductAttrAsset>(
      this.assetConfigs,
      (config) => new ProductAttrAsset(config),
    );
    ...
    this.collections = this.instantiateFromConfig<CollectionConfig, Collection<any>>(
    ...
    for (const [name, serviceConfig] of Object.entries(this.serviceConfigs)) {
      ...
      for (const family of serviceConfig.families ?? []) {
        ...
        for (const attribute of familyConfig.rules.attributes?.required ?? []) {
          ...
          familyInstance
            .getAttributeManager()
            .add(attributeInstance, undefined, { required: true });
        }
        ...
        for (const attribute of familyConfig.rules.attributes?.optional ?? []) {
          ...
          familyInstance
            .getAttributeManager()
            .add(attributeInstance, undefined, { required: false });
        }
      }
    }
  }
  ...
  protected registerCollections(): void {
    ...
    for (const config of Object.values(this.collectionConfigs)) {
      for (const value of config.values) {
        console.log(
          `Adding ${config.type} for '${value}' attribute to collection '${config.name}'...`,
        );
    ...
    for (const [serviceName, serviceConfig] of Object.entries(this.serviceConfigs)) {
      for (const [type, collectionNames] of Object.entries(serviceConfig.collections)) {
        for (const collectionName of collectionNames) {
          console.debug(
            `Registering collection '${collectionName}' of type '${type}' for service '${serviceName}'...`,
          );
  ...
  protected instantiateFromConfig<T, V>(
    configs: Record<string, T>,
    constructor: (config: T) => V,
  ): Record<string, V> {
