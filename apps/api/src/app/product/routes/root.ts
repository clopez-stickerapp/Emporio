...
export default async function (fastify: FastifyInstance) {
  ...
  f.get(
    '/minimum-quantity/:family/:name',
    { schema: getMinimumQuantitySchema },
    async function (request) {
      const item = new ProductItem(
        request.params.family,
        request.params.name,
        JSON.parse(request.query.attributes),
      );
  ...
  f.get(
    ...
    { schema: getAttributeMapSchema },
    async function (request) {
      return {
        attributes: emporio
          .getProductService()
          .getProductMap(request.params.family, request.params.name),
      };
    },
  );

  f.get(
    '/conditionable-map/:family',
    { schema: getConditionableMapSchema },
    async function (request) {
  ...
  f.get(
    ...
    { schema: isAttributeAvailableSchema },
    async function (request) {
      const itemJSON = JSON.parse(request.query.item);
      const item = new ProductItem(
        itemJSON.productFamilyName,
        itemJSON.productName,
        itemJSON.attributes,
      );
      return {
        available: emporio.isAttributeAvailable(
          item,
          request.params.name,
          request.params.value,
          request.query.useFilters,
        ),
      };
    },
  );
  ...
  f.get(
    ...
    { schema: isAttributeRequiredSchema },
    async function (request) {
      return {
        required: emporio.isAttributeRequired(
  ...
  f.get('/families', { schema: getFamiliesSchema }, async function (request) {
    ...
    for (const family of productFamilies) {
      families[family.getName()] = {
        name: family.getName(),
        supported: Object.values(family.getAttributeManager().getAll()).map((attribute) =>
  ...
  f.get('/out-of-stock/:family', { schema: getOutOfStockSchema }, async function (request) {
    const products = emporio
      .getProductService()
      .retrieveProductFamily(request.params.family)
      .getProducts();
