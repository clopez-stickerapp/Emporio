...
export class ProductService {
  ...
  public retrieveCollection<T extends CollectionItem>(
    type: CollectionType,
  ...
  public registerMinimumUnitsCollection(collection: MinimumUnitsCollection): void {
    if (this.minimumUnitsCollections[collection.getCollectionName()]) {
      throw new Error(
        'Minimum units collection already exists with name ' + collection.getCollectionName(),
      );
    }
  ...
  public registerQuantityListCollection(collection: ProductQuantityListCollection): void {
    if (this.quantityListCollections[collection.getCollectionName()]) {
      throw new Error(
        'QuantityList collection already exists with name ' + collection.getCollectionName(),
      );
    }
  ...
  public getProductMap(familyName: string, productName?: string): AttributesMap {
    ...
    const asset = this.retrieveCollection<ProductAttrAsset>(
      CollectionType.Asset,
      family.getAssetCollectionName(),
    );
    const filter = this.retrieveCollection<ProductAttrFilter>(
      CollectionType.Filter,
      family.getFilterCollectionName(),
    );
    const constraint = this.retrieveCollection<ProductAttrConstraint>(
      CollectionType.Constraint,
      family.getConstraintsCollectionName(),
    );

    for (const [attrName, { instance: attr }] of Object.entries(
      family.getAttributeManager().getAll(),
    )) {
