...
import {
  ...
  ProductionLines,
} from '$/configuration/attributes/ProductionLineAttribute';
import {
  CutDirectionAttribute,
  CutDirectionAttributeValues,
} from '$/configuration/attributes/CutDirectionAttribute';
import { DeliveryAttribute, DeliveryTypes } from '$/configuration/attributes/DeliveryAttribute';
import {
  WhiteLayerAttribute,
  WhiteLayerValues,
} from '$/configuration/attributes/WhiteLayerAttribute';
...
export class ProductionHelper {
  ...
  public unsetSettingsAutomatically(): void {
    ...
    if (
      !this.isRecommended(CutDirectionAttribute.getName()) &&
      this.item.getProductName() !== ProductNames.PRODUCT_LABELS_ON_ROLL
    ) {
  ...
  public setSettingsAutomatically(): void {
    if (
      !this.itemHelper.getProductionLine() &&
      this.attrComputer.isSupported(ProductionLineAttribute.getName())
    ) {
    ...
    if (
      !this.itemHelper.getDelivery() &&
      this.attrComputer.isSupported(DeliveryAttribute.getName())
    ) {
    ...
    if (
      !this.itemHelper.getWhiteLayer() &&
      this.attrComputer.isSupported(WhiteLayerAttribute.getName())
    ) {
    ...
    if (
      !this.itemHelper.getCutDirection() &&
      this.attrComputer.isSupported(CutDirectionAttribute.getName())
    ) {
  ...
  public detectSuitableCutDirection(): string | null {
    if (
      this.attrComputer.isAvailable(
        CutDirectionAttribute.getName(),
  ...
  private isRecommended(attributeName: string): boolean {
    if (this.attrComputer.isRequired(attributeName)) {
      ...
      const product = this.ps
        .retrieveProductFamily(this.item.getProductFamilyName())
        .getProduct(this.item.getProductName());
