...
export class SizeHelper {
  ...
  public toString(): string {
    return `|${this.width.mm}|${this.height.mm}|${this.minSize.mm}|${this.maxSize.mm}|${
      this.fixedSize
  ...
  public extractMeassuresFromString(sizeString: string): { width: number; height: number } {
    const [width, height] = (
      sizeString.match(/([\d]{1,3}\.?[\d]{1,3})|([\d]{1,3})/g)?.map(Number) ?? []
    ).slice(-2);
  ...
  public localizedToMM(measureInInchesOrCM: number): number {
    return this.isImperialUnits()
      ? convertInchesToMM(measureInInchesOrCM)
      : convertCMToMM(measureInInchesOrCM);
  }
  ...
  protected evaluateMaxSize(): void {
    let maxSizeBasedOnProductionLine: number = this.canBeProductionLine('digital')
      ? SizeHelper.MAX_SIZE_MM_DIGITAL
      : SizeHelper.MAX_SIZE_MM_LASER;
    ...
    if (this.productItem.getProductName() == 'labels_on_roll') {
      maxSizeBasedOnProductionLine =
        this.attrComputer.getHighestAvailableValue('max_size_mm') ??
        Math.max(SizeHelper.MAX_SIZE_MM_DIGITAL, SizeHelper.MAX_SIZE_MM_LASER);
    } else if (!this.attrComputer.useFilters && !this.canBeProductionLine('digital')) {
      maxSizeBasedOnProductionLine = 800;
    }
  ...
  protected evaluateMinSize(): void {
    ...
    if (!minSizeMM) {
      minSizeMM =
        this.attrComputer.getLowestAvailableValue('min_size_mm') ?? SizeHelper.MIN_SIZE_MM_DEFAULT;
    }
  ...
  protected evaluateMaxSizeOtherSide(): void {
    ...
    if (!maxSizeOtherSide) {
      ...
      if (values.length) {
        ...
        maxSizeOtherSide = SizeHelper.MAX_SIZE_MM_LASER;
      }
    }
